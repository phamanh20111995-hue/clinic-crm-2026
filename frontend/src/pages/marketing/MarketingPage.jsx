...
const TABS = [
  ...
  null,
  { k: 'kanban',   l: 'Kanban',            icon: IconLayoutKanban },
  { k: 'calendar', l: 'Content Calendar',  icon: IconCalendarMonth },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100%' }}>
      ...
      <div style={{ height: 40, background: '#fff', borderBottom: '1px solid #dde3ef', display: 'flex', alignItems: 'center', paddingLeft: 8, paddingRight: 8, gap: 2, flexShrink: 0, overflowX: 'auto' }}>
        {TABS.map((t, idx) => {
          if (t === null) return <div key={`sep-${idx}`} style={{ width: 1, height: 20, background: '#dde3ef', flexShrink: 0, margin: '0 4px' }} />
          const Icon = t.icon
          const active = tab === t.k
          return (
            <button key={t.k} onClick={() => setTab(t.k)}
              style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 10px', border: 'none', borderBottom: `2px solid ${active ? ACCENT : 'transparent'}`, background: 'transparent', color: active ? ACCENT : '#64748b', fontWeight: active ? 700 : 400, fontSize: 11, cursor: 'pointer', whiteSpace: 'nowrap', transition: 'all .12s', flexShrink: 0, fontFamily: 'inherit', height: 40, position: 'relative' }}>
              <Icon size={13} />
              {t.l}
              {t.badge && (
                <span style={{ background: '#dc2626', color: '#fff', borderRadius: 20, padding: '0 5px', fontSize: 8, fontWeight: 700, lineHeight: '14px' }}>{t.badge}</span>
              )}
            </button>
          )
        })}

        {/* Month selector + user label — hide on content tabs that don't need it */}
        {!isContentTab && (
      ...
      {/* Tab content */}
      <div style={{ flex: 1, overflowY: tab === 'kanban' ? 'hidden' : 'auto', display: 'flex', flexDirection: 'column' }}>
